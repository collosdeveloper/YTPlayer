...
import youtube.com.np.player.managers.HistoryRecordManager;
import youtube.com.np.player.managers.MediaSessionManager;
import youtube.com.np.player.managers.MediaSourceManager;
import youtube.com.np.player.mediasource.FailedMediaSource;
import youtube.com.np.player.mediasource.MediaSourceTag;
import youtube.com.np.player.mediasource.PlayerDataSource;
import youtube.com.np.player.playqueue.PlayQueue;
import youtube.com.np.player.playqueue.PlayQueueAdapter;
...
import youtube.com.np.utils.SerializedCache;
...
public abstract class BasePlayer implements
	...
	public BasePlayer(@NonNull final Context context) {
		...
		this.recordManager = new HistoryRecordManager();
	...
	public void initPlayer(final boolean playOnReady) {
		LOGD(TAG, "initPlayer() called with: context = [" + context + "]");
	...
	public void initPlayback(@NonNull final PlayQueue queue,
	...
	private void initThumbnail(final String url) {
		LOGD(TAG, "Thumbnail - initThumbnail() called");
	}
	...
	public void unregisterBroadcastReceiver() {
		try {
			...
			LOGE(TAG, "Broadcast receiver already unregistered.", unregisteredException);
		}
	}
	...
	public void onRepeatClicked() {
		...
		setRepeatMode(mode);
		LOGD(TAG, "onRepeatClicked() currentRepeatMode = " + getRepeatMode());
	}
	...
	private Disposable getProgressReactor() {
		return Observable.interval(PROGRESS_LOOP_INTERVAL_MILLIS, TimeUnit.MILLISECONDS)
				...
				.subscribe(ignored -> triggerProgressUpdate(),
						error -> LOGE(TAG, "Progress update failure: ", error));
	}
	...
	public void onTimelineChanged(Timeline timeline, Object manifest,
		...
		LOGD(TAG, "ExoPlayer - onTimelineChanged() called with " +
	...
	public void onTracksChanged(TrackGroupArray trackGroups, TrackSelectionArray trackSelections) {
		LOGD(TAG, "ExoPlayer - onTracksChanged(), " +
	...
	public void onPlaybackParametersChanged(PlaybackParameters playbackParameters) {
		LOGD(TAG, "ExoPlayer - playbackParameters(), " +
	...
	public void onLoadingChanged(final boolean isLoading) {
		LOGD(TAG, "ExoPlayer - onLoadingChanged() called with: " +
				"isLoading = [" + isLoading + "]");
	...
	public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
		LOGD(TAG, "ExoPlayer - onPlayerStateChanged() called with: " +
				"playWhenReady = [" + playWhenReady + "], " +
				"playbackState = [" + playbackState + "]");
		
		if (getCurrentState() == STATE_PAUSED_SEEK) {
			LOGD(TAG, "ExoPlayer - onPlayerStateChanged() is currently blocked");
			return;
		}
	...
	private void maybeCorrectSeekPosition() {
		...
		if (presetStartPositionMillis > 0L) {
			...
			LOGD(TAG, "Playback - Seeking to preset start " +
	...
	public void onPlayerError(ExoPlaybackException error) {
		LOGD(TAG, "ExoPlayer - onPlayerError() called with: " +
				"error = [" + error + "]");
		if (errorToast != null) {
			errorToast.cancel();
			errorToast = null;
	...
	public void onPositionDiscontinuity(@Player.DiscontinuityReason final int reason) {
		LOGD(TAG, "ExoPlayer - onPositionDiscontinuity() called with " +
				"reason = [" + reason + "]");
	...
	public void onRepeatModeChanged(@Player.RepeatMode final int reason) {
		LOGD(TAG, "ExoPlayer - onRepeatModeChanged() called with: " +
				"mode = [" + reason + "]");
	}
	
	@Override
	public void onShuffleModeEnabledChanged(final boolean shuffleModeEnabled) {
		LOGD(TAG, "ExoPlayer - onShuffleModeEnabledChanged() called with: " +
	...
	public void onPlaybackSynchronize(@NonNull final PlayQueueItem item) {
		LOGD(TAG, "Playback - onPlaybackSynchronize() called with " +
		...
		if (currentPlayQueueIndex != playQueue.getIndex()) {
			LOGE(TAG, "Playback - Play Queue may be desynchronized: item " +
			...
			LOGE(TAG, "Playback - Trying to seek to invalid " +
					"index=[" + currentPlayQueueIndex + "] with " +
					"playlist length=[" + currentPlaylistSize + "]");
			
		} else if (currentPlaylistIndex != currentPlayQueueIndex || onPlaybackInitial ||
				!isPlaying()) {
			LOGD(TAG, "Playback - Rewinding to correct" +
					" index=[" + currentPlayQueueIndex + "]," +
	...
	protected void onMetadataChanged(@NonNull final MediaSourceTag tag) {
		...
		LOGD(TAG, "Playback - onMetadataChanged() called, playing: " + info.getName());
	...
	public void onFastRewind() {
		LOGD(TAG, "onFastRewind() called");
		seekBy(-FAST_FORWARD_REWIND_AMOUNT_MILLIS);
	}
	
	public void onFastForward() {
		LOGD(TAG, "onFastForward() called");
		seekBy(FAST_FORWARD_REWIND_AMOUNT_MILLIS);
	}
	...
	protected void savePlaybackState(final StreamInfo info, final long progress) {
		if (info == null) return;
		recordManager.saveStreamState(info, progress);
	}
