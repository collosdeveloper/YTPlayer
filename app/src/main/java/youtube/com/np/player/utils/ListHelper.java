...
import static youtube.com.utils.PrefHelper.Keys.DEF_AUDIO_FORMAT_KEY;
import static youtube.com.utils.PrefHelper.Keys.DEF_VIDEO_FORMAT_KEY;
import static youtube.com.utils.PrefHelper.Keys.MOBILE_DATA_USAGE_KEY;
import static youtube.com.utils.PrefHelper.Keys.SHOW_HIGHER_RESOLUTION_KEY;
...
public final class ListHelper {
	...
	public static int getDefaultAudioFormat(Context context, List<AudioStream> audioStreams) {
		MediaFormat defaultFormat = getDefaultFormat(context, DEF_AUDIO_FORMAT_KEY,
	...
	public static List<VideoStream> getSortedStreamVideosList(Context context, List<VideoStream> videoStreams, List<VideoStream> videoOnlyStreams, boolean ascendingOrder) {
		boolean showHigherResolutions = PrefHelper.get(SHOW_HIGHER_RESOLUTION_KEY, false);
		MediaFormat defaultFormat = getDefaultFormat(context, DEF_VIDEO_FORMAT_KEY, R.string.default_video_format_value);
	...
	private static String computeDefaultResolution(Context context, int key, int value) {
		...
		String resolution = PrefHelper.get(context.getString(key), context.getString(value));
	...
	private static MediaFormat getDefaultFormat(Context context, String defaultFormatKey, @StringRes int defaultFormatValueKey) {
		
		String defaultFormat = context.getString(defaultFormatValueKey);
		String defaultFormatString = PrefHelper.get(defaultFormatKey, defaultFormat);
		
		MediaFormat defaultMediaFormat = getMediaFormatFromKey(context, defaultFormatString);
		if (defaultMediaFormat == null) {
			PrefHelper.save(defaultFormatKey, defaultFormat);
	...
	private static String getResolutionLimit(Context context) {
		...
		if (!isWifiActive(context)) {
			...
			String value = PrefHelper.get(MOBILE_DATA_USAGE_KEY, defValue);
