...
public class CacheFactory implements DataSource.Factory {
	...
	public DataSource createDataSource() {
		LOGD(TAG, "initExoPlayerCache: cacheDir = " + cacheDir.getAbsolutePath());
	...
	public void tryDeleteCacheFiles() {
		...
		try {
			for (File file : cacheDir.listFiles()) {
				...
				LOGD(TAG, "tryDeleteCacheFiles: " + filePath + " deleted = " + deleteSuccessful);
			}
		} catch (Exception ignored) {
			LOGE(TAG, "Failed to delete file.", ignored);
		}
	}
}