	...
	private Subscriber<PlayQueueEvent> getSelfReporter() {
		return new Subscriber<PlayQueueEvent>() {
			...
			public void onNext(PlayQueueEvent event) {
				LOGD(TAG, "Received broadcast: " + event.type().name() + ". Current index: " + getIndex() + ", play queue length: " + size() + ".");
				reportingReactor.request(1);
			}
			
			@Override
			public void onError(Throwable t) {
				LOGE(TAG, "Received broadcast error", t);
			}
			
			@Override
			public void onComplete() {
				LOGD(TAG, "Broadcast is shutting down.");
			}
		};
	}
}