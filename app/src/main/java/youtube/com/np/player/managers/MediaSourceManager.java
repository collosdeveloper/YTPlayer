...
import youtube.com.np.player.PlaybackListener;
import youtube.com.np.player.mediasource.FailedMediaSource;
import youtube.com.np.player.mediasource.LoadedMediaSource;
import youtube.com.np.player.mediasource.ManagedMediaSource;
import youtube.com.np.player.mediasource.ManagedMediaSourcePlaylist;
import youtube.com.np.player.mediasource.PlaceholderMediaSource;
...
import youtube.com.np.player.playqueue.events.ReorderEvent;
import youtube.com.np.utils.ServiceHelper;

import static youtube.com.np.player.mediasource.FailedMediaSource.MediaSourceResolutionException;
import static youtube.com.np.player.mediasource.FailedMediaSource.StreamInfoLoadException;
...
public class MediaSourceManager {
	...
	private void maybeLoadItem(@NonNull final PlayQueueItem item) {
		...
		if (!loadingItems.contains(item) && isCorrectionNeeded(item)) {
			LOGD(TAG, "MediaSource - Loading=[" + item.getTitle() +
	...
	private Single<ManagedMediaSource> getLoadedMediaSource(@NonNull final PlayQueueItem stream) {
		return stream.getStream().map(streamInfo -> {
			...
			final long expiration = System.currentTimeMillis() +
					ServiceHelper.getCacheExpirationMillis();
	...
	private void onMediaSourceReceived(@NonNull final PlayQueueItem item,
		...
		LOGD(TAG, "MediaSource - Loaded=[" + item.getTitle() +
				"] with url=[" + item.getUrl() + "]");
		...
		if (isCorrectionNeeded(item)) {
			LOGD(TAG, "MediaSource - Updating index=[" + itemIndex + "] with " +
	...
	private void maybeRenewCurrentIndex() {
		...
		LOGD(TAG, "MediaSource - Reloading currently playing, " +
